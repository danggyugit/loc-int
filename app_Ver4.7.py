# app_Ver4.7.py — 입지 선정 분석 툴 v4.7 (Streamlit 앱)
# 실행: streamlit run app_Ver4.7.py
#
# v4.6 대비 개선 사항 (묶음 B: 정확성 패치):
#   1. _merge_gdfs에 dedup_col 파라미터 추가 — 다중 구 분석 시
#      경계가 인접한 구(예: 강남↔서초)에서 카카오 반경 검색이 겹쳐
#      같은 매장이 두 번 카운트되던 문제 해결
#   2. 카카오 소스 6종(competitor/transport/population/parking/diversity/
#      buildings)에 dedup 적용 — 경쟁업체 카운트·교통 점수의 과대 계상 방지
#
# 이전 버전(v4.6) 기능 유지:
#   - PRESETS/SIDO_GU_MAP config.py 중앙화
#   - API 키 입력 단일 expander 통합
#   - 시/도 → 구/군/시 2단계 드롭다운 + 다중 구 통합 분석
