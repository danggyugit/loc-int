# app_Ver3.0.py — 입지 선정 분석 툴 v3.0 (Streamlit 앱)
# 실행: streamlit run app_Ver3.0.py
#
# v2 대비 개선 사항:
#   1. SGIS 인구통계 지표 (평균 연령, 유소년/고령 부양비) 수집 및 타겟 연령 가중
#   2. 주차 인프라 (카카오 키워드 '주차장') 신규 팩터 추가
#   3. 6팩터 점수 모델 (인구·유동·직장·경쟁·접근성·주차)
#   4. demographic_target 프로파일: children / elderly / young_adult / all
...
if not run_btn and st.session_state["analysis_cache"] is None:
    ...
    **v3.0 개선 사항**
    - 타겟 연령대 인구 가중: 업종별 유소년/고령/생산가능인구 비중 반영
    - 주차 인프라 팩터 추가: 차량 방문형 업종에 주차장 밀도 반영
    - 6팩터 점수 모델: 인구·유동·직장·경쟁·접근성·주차
    """)
    st.stop()
...
c6.metric("핫스팟 클러스터", f"{n_hotspot}개")
c7.metric("경쟁 공백 지역",  f"{n_gap}셀")
...
# ── 지도 빌드 ────────────────────────────────────────────
from src.visualizer import build_combined_folium_map
...
display_cols = ["rank", "grid_id", "population", "floating", "workplace", "competitor_cnt", "transport_cnt", "parking_cnt", "score"]
