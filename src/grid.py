...
def build_grid_features(
    ...
    Returns:
        컬럼: grid_id, population, floating, workplace,
              competitor_cnt, transport_cnt, parking_cnt,
    ...
    if population_gdf is not None:
        ...
        # 인구통계 지표: 포인트의 가중 평균 (인구 배분 포인트에 동일 값이므로 mean 사용)
    ...
    if transport_gdf is not None:
        grid = aggregate_to_grid(grid, transport_gdf, None, "transport_cnt", agg="count")
    else:
        grid["transport_cnt"] = 0
