# src/scoring_Ver4_2.py — 9팩터 점수화 모델 (AA-S Sub-agent)
#
# v4.0 대비 변경 사항:
#   1. 소득 수준(income) 팩터 추가 — 양의 기여 (구매력 높은 지역 선호)
#   2. 임대 비용(rent) 팩터 추가 — 음의 기여 (높은 임대료 = 비용 페널티)
#   3. children_and_parent 인구통계 타겟 추가 (5~13세 + 35~45세 혼합)
#   4. 9팩터 가중치 모델: 인구·유동·직장·경쟁·접근성·주차·다양성·소득·임대
#   5. 데이터 미제공 팩터의 가중치 자동 재분배
...
PROFILES_FILE = PRESETS_DIR / "scoring_profiles_v42.yaml"
...
def load_profile(preset: str | None = None, keyword: str | None = None) -> dict:
    """
    scoring_profiles_v42.yaml에서 프로파일 로드.
    ...
    # 하위 호환: 7팩터 프로파일에 income/rent가 없으면 0.0 추가
...
def _calc_demographic_modifier(
    ...
    v4.2 추가 타겟:
        children_and_parent — 유소년(5~13세 proxy) + 부모세대(35~45세 proxy) 혼합
                              juv_suprt_per × 0.6 + young_adult × 0.4
    """
    ...
    if demographic_target == "children":
        ...
        #      생산가능인구(15~64)에서 부모세대(35~45) 비율은 ~30%이지만,
...
def _redistribute_weights(
    ...
    """
    데이터가 없는 팩터(컬럼이 없거나 전부 0)의 가중치를
    나머지 팩터에 비례 배분.

    Why: income/rent 데이터가 없는 환경에서도 7팩터 모델처럼
         정상 동작하려면 빈 팩터의 가중치를 다른 팩터로 이전해야 함.
    """
    col_map = {
...
def calc_score(
    ...
    """
    격자 GeoDataFrame에 종합 점수(score) 컬럼 추가 (9팩터).
    ...
    수식:
        score = pop_adjusted  * w_population
              ...
              + income_norm   * w_income        (소득 높을수록 유리)
              - rent_norm     * w_rent          (월세 높을수록 비용 페널티)
    """
    out = gdf.copy()
    ...
    income_label = f"|income={weights['income']:.0%}" if weights["income"] > 0 else ""
    rent_label = f"|rent={weights['rent']:.0%}" if weights["rent"] > 0 else ""
    log.info(
        f"[{profile['profile_key']}|{comp_mode}{demo_label}{income_label}{rent_label}] "
...
def score_and_rank(
    ...
    """
    격자 GDF에 9팩터 점수화 + 순위 산출을 한 번에 수행.
