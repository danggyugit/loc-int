# app_Ver4.8.py — 입지 선정 분석 툴 v4.8 (Streamlit 앱)
# 실행: streamlit run app_Ver4.8.py
#
# v4.7 대비 개선 사항 (묶음 C+D: 캐싱·UX):
#   1. 캐시 TTL 1시간 → 24시간 — 같은 분석을 하루 안에 재실행 시
#      네트워크 호출 없이 즉시 반환
#   2. st.progress → st.status 전환 — 단계별 진행 사항이 그룹화되어
#      누적 표시되고 완료 시 자동 접힘
#   3. 에러 메시지 컨텍스트 추가 — 어느 단계에서 실패했는지 표시하고
#      흔한 원인(API 키, 타임아웃, 지역명 인식 실패)에 대한 힌트 제공
#
# 이전 버전(v4.7) 기능 유지:
