# app_Ver4.9.py — 입지 선정 분석 툴 v4.9 (Streamlit 앱)
# 실행: streamlit run app_Ver4.9.py
#
# v4.8 대비 개선 사항 (공개 배포 블로커 해소):
#   1. 세션별 API 키 격리 — session_keys 모듈로 전환하여
#      사용자 A의 키가 사용자 B의 세션으로 새는 문제 해결
#      (Streamlit Cloud 프로세스 공유 환경에서 치명적이던 os.environ 오염 제거)
#   2. 분석 범위 상한 — 예상 셀 수가 임계치 초과 시 경고 + 차단
#      악의·부주의 입력으로 인한 API 쿼터 소진·OOM 방지
#   3. 타이틀·랜딩 안내 v4.9로 갱신
#
# 이전 버전(v4.8) 기능 유지:
